...
pub fn clone_repo((url, target_dir): (&str, &str)) -> Result<()> {
    ...
    if res_data_size < HASH_BYTES {
        bail!("Data length is to short. Size: {:?}", res_data_size);
    }

    let res_data_size = res_data_size - HASH_BYTES;
    //println!("res_data_size: {:?}", res_data_size);

    let entries_bytes = res_data
        .get(16..20)
        .ok_or(anyhow!("Size data do not exist!"))?
        .try_into()?;
    ...
    let tree_sha = data
        .clone()
        .last()
        .ok_or(anyhow!("Data on last index do not exist!"))?;
...
fn undeltified(delta: &[u8], base: &[u8]) -> Result<Vec<u8>> {
    ...
    while seek < delta_len {
        let instr_byte = *delta
            .get(seek)
            .ok_or(anyhow!("Data on this index do not exist!"))?;
        seek += 1;
        ...
        if instr_byte >= 128 {
            ...
            content.extend_from_slice(
                base.get(offset..(offset + len_int))
                    .ok_or(anyhow!("No data in indexing area"))?,
            );
...
fn checkout_tree(sha: &str, file_path: &str, target_dir: &str) -> Result<()> {
    ...
    let pos = v_git_data
        .iter()
        .position(|&r| r == b'\x00')
        .ok_or(anyhow!("Position is not exist"))?;
    ...
    while !tree.is_empty() {
        ...
        let sha = &tree
            .get(..HASH_BYTES)
            .ok_or(anyhow!("No data in sha area"))?;

        tree = tree
            .get(HASH_BYTES..)
            .ok_or(anyhow!("No data in data area"))?;
    ...
    for entry in enteries {
        ...
        if entry.0 == "40000" {
            ...
            let content = &v_git_data
                .get(pos + 1..)
                .ok_or(anyhow!("Elements not found"))?;

            let new_path = file_path.to_owned() + &format!("/{}", entry.1);
