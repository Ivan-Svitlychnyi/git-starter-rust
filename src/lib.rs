...
pub fn write_tree(file_path: &PathBuf) -> Result<String> {
    ...
    let mut entries = fs::read_dir(file_path)?
        ...
        .filter(|path| path.as_ref().unwrap().file_name().unwrap() != ".git")
    ...
    for dir in entries {
        ...
        /*if dir.is_file()*/
        {
            ...
            bail!("This path is not relevant");
        }
        ...
        dir_sha_out.extend_from_slice(
            dir.file_name()
                .ok_or(anyhow!("The file name is invalid"))?
                .to_str()
                .ok_or(anyhow!("The file name is not converted to str"))?
                .as_bytes(),
        );
